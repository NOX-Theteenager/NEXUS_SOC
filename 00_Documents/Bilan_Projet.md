...
**Date :** 2 juillet 2026 — mi-parcours du stage MINFI
...
### Cœur technique
- **Lot 0 — Socle** : Docker Compose (Kafka + TimescaleDB + Wazuh), schémas SQL avec RLS multi-tenant, retention policies, seed démo
...
- **Lot 6 — Tests** : 18/18 passent, couvre Auth, Admin, Analyste, PLG
- **Lot 7 — Console Fournisseur** : Admin (/admin), Analyste (/analyst), Portail DSI (/portal), Provisioning agents (/provision)
- **Lot 8 — PLG** : registration OTP, vérification, pré-autorisation CinetPay, upgrade abonnement, quotas essai
- **Lot 9 — Frontend PWA** : landing 3D, login, console, portail, docs, contact, manifest, service worker, offline

### Déploiement
- **HTTPS public** : `https://nexussoc.cm` + `https://api.nexussoc.cm` via Cloudflare Tunnel (certificat Google Trust Services, HSTS actif)
- **Systemd** : `cloudflared` + `nexus-soc` auto-start au boot, restart en cas de crash
- **Sécurité** : HSTS + X-Frame-Options + Content-Type-Options + Referrer-Policy en réponse

### Intégrations externes
- ✅ **Gmail SMTP** — OTP par e-mail via `nguetsajunior@gmail.com` (testé et fonctionnel)
- ✅ **CinetPay** — pré-autorisation en mode sandbox, webhook signé HMAC
- ✅ **VirusTotal** — enrichissement IOC dans les 4 playbooks (mode désactivé par défaut, activable sans redémarrage)

### Documentation
- `README.md` — vue d'ensemble
- `NEXUS_SOC_Contexte_Complet.md` — contexte projet + décisions
- `00_Documents/Configuration_Services_Externes.md` — guide config Gmail/CinetPay/VirusTotal
- `00_Documents/Deploiement_HTTPS_nexussoc.md` — guide Cloudflare Tunnel
...
- **CinetPay production** : passer de sandbox à live après validation du compte marchand (72h)
- **Compte marchand CinetPay validé** → activer les 3 plans (Starter/Business/Enterprise) sur le portail public
- **Domaine e-mail** : configurer SPF/DKIM/DMARC sur `nexussoc.cm` pour envoyer depuis `no-reply@nexussoc.cm` au lieu de `nguetsajunior@gmail.com`
- **Wazuh Manager** : brancher l'ingestion de vraies alertes système (SSH, sudo, escalade)

### Moyen terme
- **Stack Kafka + Wazuh** au démarrage : quand le PC de démo a assez de RAM (>8 Go), lancer tout le docker-compose au boot au lieu de juste PostgreSQL
- **LLM Analyst** : brancher Ollama local (Mistral) pour explication des alertes en langage naturel
- **Tests d'intégration** : couvrir SMTP, CinetPay webhook, VirusTotal
- **Monitoring Prometheus + Grafana** : dashboards de santé pour l'exposé technique

### Long terme (post-soutenance)
- **Multi-région Kubernetes** : passer de mono-machine à cluster
- **Marketplace de playbooks** : que les tenants puissent partager leurs SOAR customs
- **Certification ANTIC/COBAC** : dossier de conformité pour vendre à la vraie MINFI
...
| Endpoints API | 42 (Auth, Admin, Analyst, Portal, Provision, PLG) |
| Playbooks SOAR | 4 |
| Modèles IA | 2 (Isolation Forest + UEBA) |
| Tests automatisés | 18 (100% verts) |
| Intégrations externes | 3 ításpont (Gmail, CinetPay, VirusTotal) |
...
- **Ouvrir `https://nexussoc.cm`** depuis n'importe quel appareil
- **Landing page 3D + animations parallax** → montre la maturité produit
- **Docs interactive** (`/app/docs.html`) → 8 sections avec exemples
- **Contact** (`/app/contact.html`) → formulaire prospect
- **Inscription PLG** avec **OTP réel Gmail** → montre le flux SaaS end-to-end
- **Portail DSI** (login `dsi@minfi.cm` / `admin`) → 2 alertes MINFI + notifications + rapports téléchargeables
- **Console admin** (login `admin@nexussoc.cm` / `admin`) → tenants, agents, KPIs
